								...
								)
{
	...
	getCurrentReference( startTime,yRef,xEst );
	#ifdef SIM_DEBUG
										...
										)
{
	...
	VariablesGrid yRef( _yRef );
	getCurrentReference( currentTime,yRef, _y);
	#ifdef SIM_DEBUG
											...
											)
{
	...
	DVector x_dummy;
	VariablesGrid yRef( _yRef );
	getCurrentReference( nextTime,yRef,x_dummy);
	#ifdef SIM_DEBUG
												...
												) const
{
	...
	{
		if ( referenceTrajectory != 0 )
			referenceTrajectory->getReference( tStart,tEnd, _yRef,x_check );
	}
